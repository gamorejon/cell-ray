...
public:
	...
	{
	    vector float depth = negativeZero();
	    vector float increment = vectorOne();
		...
		vector float currentReflection = splatScalar(1.0f);
...
//			vec_any_gt(currentReflection, vectorEpsilon()) &&
              ...
              vec_all_gt(color->b, increment)))
	    {
    	    ...
    	    if (vec_any_ne(intersection.hit, boolZero()))
        	{
				...
				currentReflection = vec_and(intersection.hit, 
					currentReflection);
	
    	        color->addColor(*surface, currentReflection);
        	    color->addColor(*ambient, currentReflection);
