...
const float START_CAMERA_DISTANCE = 0.40f;
const float END_CAMERA_DISTANCE = 0.40;
...
Scene* getScene(float theta);

RayTraceState* getState(ColorGroup* pixels, Scene* scene, float camera, float theta);
...
float cameraDistance = START_CAMERA_DISTANCE;

float delta = (END_CAMERA_DISTANCE - START_CAMERA_DISTANCE) / FRAMES;

float theta = 0;

float thetaDelta = 3.141592653589793238462643383279502884197169399375105820974944592 / FRAMES;

for (int loop = 0; loop < 2; loop++)
{
for (int fileNum = 0; fileNum <= FRAMES; fileNum++)
{
	char fileName[13];

	sprintf(fileName, "file%.4i.bmp", fileNum + loop*FRAMES);
...
//	printf("Original camera: (%f, %f, %f), %p\n", state->camera.x, state->camera.y, state->camera.z, state);
	...
	for (int i = 0, offset = 0; i < spe_threads; i++, offset += count)
	{
		...
		RayTraceState* state = getState(pixels + offset, scene, cameraDistance, 0);
    ...
    printf("Finished file %s (dist: %f).  Total time: %li (state: %li, spu creation: %li, spu execution: %li, processing: %li, file: %li)\n", fileName, cameraDistance, time, stateTime, spuCreationTime, totalSpuTime, processingTime, fileWritingTime);
	...
	{
		RayTraceState* state = (RayTraceState*)datas[i].argp;
		delete state;
	}
...
Scene* getScene(float theta)
{
    ...
    objects[3] = new Sphere(radius, new Vertex(-a,  a,  a),
    	reflective, new Color(c, c, c), specularReflection);
    // center sphere
    float r2 = 0.5*(sqrt(0.75) - radius);
    
    objects[4] = new Sphere(r2, new Vertex(-sin(theta)*r2, cos(theta)*r2, 0),
        reflective, new Color(c, c, c), specularReflection);
...
RayTraceState* getState(ColorGroup* pixels, Scene* scene, float cameraPos,
	float rotationAngle)
{
	...
	float newX = yBasis->x * cosTheta + xBasis->x * sinTheta;
	float newY = yBasis->y * cosTheta + xBasis->y * sinTheta;
	yBasis->z  = yBasis->z * cosTheta + xBasis->z * sinTheta;
	yBasis->y = newY;
	yBasis->x = newX;
    ...
    RayTraceState* state = new RayTraceState(camera, xBasis, yBasis, zBasis, 
